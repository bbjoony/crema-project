...
**→ 이 예측이 실측으로 확정됐다 (2026-08-12).** 켜둔 채 하룻밤을 넘겼더니 완전 방전.
24시간을 못 버틴다. `Keep screen on` 으로 CPU 가 잠들지 않는 것이 원인이고 낡은 셀이
겹쳤다. **상시 전원 연결을 전제로 확정**했으며, 따라서 **A / B 판단에서 전력 항목이
빠진다** — 코드량과 라이선스만 보고 고르면 된다. 상세는 CLAUDE.md 의 전원 절 참조.

**사전 점검 결과 (2026-08-11 실기, `cremaclock` 프로브):**

로드·표시를 막을 만한 것은 없었다. 상세는 CLAUDE.md 의 "C안을 안전하게 하기 위한
사전 점검" 절. 요약하면 — `require("frontend/datetime")` 이 정상적으로 풀리고,
`os.date("%a")` 가 한국어 로케일에서도 `Tue` 로 나와 번역 조회(`화요일`)가 맞아떨어진다.
`PLUGIN_ROOT` 만 닿지 않는데 이는 아래 예상 그대로이고 폴백 로고는 열린다.

**실측 환경값** (digitalclock 레이아웃 판단에 쓸 것):
화면 `1072x1448 @ 300dpi`, 170pt `00:00` 높이 `423px`, 40pt 날짜 높이 `107px`.
세로 합 423 + 여백 130 + 107 = 660 / 1448 로 여유 있음. **폭은 미측정** — 가로 1072 가
더 빡빡한 쪽이다.

**기타 주의:**

- `PLUGIN_ROOT = "plugins/digitalclock.koplugin/"` 상대 경로 — 안드로이드는 CWD 가
  `/data/data/org.koreader.launcher/files` 라 외부 저장소에 닿지 않는다. **2026-08-11
  실측으로 확인됨.** 사용자 이미지 기능은 동작하지 않고 기본 로고로 폴백한다
...
- **AGPL-3.0 이므로 파생물도 AGPL-3.0 + 출처 표기 필요.** 현재 크레마 저장소에는
  라이선스 파일이 없으므로 추가해야 한다. **B(포크) 확정으로 이건 이제 선택이 아니라
  의무다** — 코드 반입 커밋에 `LICENSE` 를 함께 넣는다
...
| 자립성 | 기기 하나로 끝남 | 플러그인은 얇은 뷰어에 그침 |

**온디바이스 렌더링이 기본 선택지**다. 캘린더 소스를 외부에서 끌어와야 할 때만 서버 방식을 재검토한다.

> 원래는 "처음부터 직접 작성한다"는 방향성이 이 판단의 근거였다. **B(포크) 확정으로 그
> 근거는 사라졌지만 결론은 그대로다** — 벽에 걸어둘 물건에 상시 구동 서버를 딸리게 하지
> 않는 편이 낫고, 베이스로 삼은 digitalclock 자체가 온디바이스다.
...
5. ~~**digitalclock.koplugin 재사용 결정**~~ — **B(포크) 확정 (2026-08-12).** C안 실측에서 수정 없이 정상 구동했고 남은 수정 목록이 작아 293줄이 그대로 쓸 값어치가 있음이 확인됐다. 상시 전원 전제로 바뀌며 전력이 기준에서 빠진 것이 A 의 장점을 지웠다. 상세는 CLAUDE.md 재사용 범위 절
6. **포크 반입** ← **다음.** 코드를 가져오는 커밋에 `LICENSE`(AGPL-3.0)와 원저작자 표기를 함께 넣는다
7. **요구사항 확정** — 표시 정보(시각/날짜/일정), 갱신 주기, 캘린더 소스(로컬 하드코딩 vs iCal), 레이아웃
8. **수정 착수** — 포크본을 뼈대로. 날짜 어순 한국어화가 첫 대상
