...
plugins/cremadisplay.koplugin/ ← 산출물. digitalclock 포크
    ...
    image.png                  표시할 이미지 (선택). 없으면 KOReader 로고로 폴백
...
10. **cremadisplay 실기 구동** ← **다음.** 아래 절 참조. 이미지 파일도 같이 넣는다
...
## cremadisplay 실기 구동 — 확인할 것 (2026-08-12 작성, 미검증)

코드는 로컬 구문 검사(luaparse 5.1)만 통과한 상태다. **기기에서 아직 한 번도 돌지
않았다.** 왕복이 비싸므로 한 번에 아래를 모두 본다.

투입: `cremadisplay.koplugin` 폴더를 위 배포 경로에 넣는다. **이미지를 쓰려면 같은
폴더에 `image.png`** (또는 jpg/jpeg/svg) 로 넣는다. 없으면 KOReader 로고로 폴백한다.

| 확인 | 실패하면 |
|---|---|
| 메뉴에 `Crema display` 가 뜨는가 | 로드 실패. 남의 코드가 아니므로 이번엔 원인을 좁힐 수 있다 |
| 날짜가 `2026년 8월 12일 화요일` 인가 | `_getDateString()` |
| 이미지가 내 것으로, 작게 나오는가 | 절대 경로(`PLUGIN_ROOT`) 또는 `IMAGE_BOX` |
| **분이 바뀔 때 옛 숫자가 안 남는가** | 중앙 정렬 계산. **가장 의심스러운 항목** |
| 화면 가장자리에 테두리가 없는가 | `FrameContainer` 의 bordersize/padding |
| 충전기를 꽂은 채 잔량이 유지되는가 | `cremaclock` 배터리 스냅샷으로 1시간 간격 두 번 |
| **충전 중에도 sleep 으로 들어가지 않는가** | 아래 절 참조 |
| 밤새 부분 갱신 뒤 잔상이 남는가 | 남으면 주기적 전체 갱신을 넣어야 한다 |

### sleep 진입 — 앱이 앞에 있는 동안만 막힌다 (2026-08-12 제기, 미검증)

충전 중인데도 기기가 sleep 으로 들어가는 것이 관찰됐다. **KOReader 를 띄우지 않은
상태의 관찰이며, 시계 구동 시에도 그럴 것이라는 뜻은 아니다.**

`Keep screen on` 은 안드로이드의 `FLAG_KEEP_SCREEN_ON` 을 쓰는데, 이 플래그는
**창 단위**라 그 창이 화면 앞에 있는 동안만 효력이 있다. 즉 크레마 홈이나 자체
뷰어가 앞에 있으면 OS 기본 절전이 그대로 작동하고, 지금 본 것이 그 상황일 가능성이
높다. 반대 근거도 있다 — **8/11 밤 digitalclock 은 잠들지 않았다.** 잠들었다면
배터리가 그렇게 빠질 이유가 없다.

→ 결론은 "시계를 **앞에 띄워둔 채**로 두면 되고, 홈으로 나오면 안 된다" 로 보인다.
전용 벽시계 용도라 제약이 되지 않는다. 다만 추정이므로 **꽂아둔 채 30분 방치해
확인한다.** 만약 그래도 잠들면 크레마 자체 절전이 플래그를 무시하는 것이고, 그때는
KOReader 의 sleep 화면 설정과 자동 절전 항목을 다시 본다.

코드 쪽은 어느 쪽이든 안전하다. `onSuspend` 에서 갱신 예약을 해제하고 `onResume`
에서 즉시 다시 그리므로, 잠들었다 깨어나도 옛 시각이 남지 않는다.

**분 갱신은 눈으로 1분만 기다리면 판정된다.** 이미지 크기를 바꾸면서 갱신 영역이
따라 움직이도록 고친 부분이라, 여기가 어긋나면 시계로 못 쓴다. 먼저 볼 것.
