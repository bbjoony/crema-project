...
- **현재 상태**: KOReader 설치·e-ink 확인 완료, digitalclock 실기 구동 성공. 포크 반입은 아직.
...
6. ~~**digitalclock 실기 구동**~~ — 성공 (2026-08-11 23:00). **배터리 실측은 방전으로 종료 (2026-08-12), 상시 전원 연결 전제로 전환.** 잔상 실측은 충전 후 재개
7. ~~**A / B 결정**~~ — **B(포크) 확정 (2026-08-12).** 아래 재사용 범위 절 참조
8. **포크 반입** ← **다음.** digitalclock 293줄을 저장소로 가져오고, 같은 커밋에 `LICENSE`(AGPL-3.0)와 원저작자 표기를 함께 넣는다
9. 요구사항 확정 (표시 정보, 갱신 주기, 캘린더 소스, 레이아웃) → 수정 착수
...
- 저장소 라이선스가 **AGPL-3.0** 이 된다. 코드를 반입하는 커밋에 `LICENSE` 와
  원저작자([DucNg](https://github.com/DucNg/digitalclock.koplugin)) 표기를 **같이** 넣는다.
  아직 코드를 안 가져왔으므로 지금은 파생물이 아니다 — 반입 시점에 함께 처리한다
...
- 대신 **"화면 켠 채로 충전이 유지되는가"** 를 확인해야 한다. 화면을 켜둔 소비가 충전
  전류를 넘으면 꽂아둬도 서서히 준다. 스냅샷을 시작·1시간 뒤로 두 번 찍어 잔량이
  오르거나 유지되는지 본다 (`charging: true` 확인 포함)
- 낡은 리튬 셀을 방전까지 두지 않는다. 완충 상태로 상시 연결하는 편이 낫다

어느 안이든 공통으로 해야 하는 일:

- **날짜 형식을 한국어 어순으로** (위 참조)
