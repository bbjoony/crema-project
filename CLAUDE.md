...
6. **digitalclock 실기 구동** — 성공 (2026-08-11 23:00). 배터리·잔상 실측 진행 중 ← **다음: 밤샘 결과 확인 후 A / B 결정**
7. 요구사항 확정 (표시 정보, 갱신 주기, 캘린더 소스, 레이아웃) → 플러그인 작성 착수

### digitalclock 재사용 범위 — C안 진행 중 (2026-08-11)

세 안 중 **C(먼저 그대로 설치해 실측)** 로 간다. C 는 A·B 어느 쪽도 닫지 않는 선택이며,
실측 없이는 "293줄을 포크할 값어치가 있는가"를 판단할 근거가 없다. **A 와 B 중 무엇을
택할지는 여전히 열려 있고, 실측 후에 결정한다.** 상세 분석은
...
| **C. 먼저 그대로 설치** ← 진행 중 | 수정 없이 올려 상시 표시·전력·레이아웃을 체감한 뒤 결정 | 결정이 늦어짐 / 실측 정보를 얻음 |

B 를 고르면 이 문서 위쪽의 **"처음부터 직접 작성 — 기존 플러그인 재사용 아님"** 방향과
라이선스 전제가 함께 바뀐다. 그때 가서 명시적으로 갱신할 것.
